...
func _process(_delta: float) -> void:
	...
	# Clamp mouse to visible screen bounds
	if player_input_device == "key":
		var viewport_rect = get_viewport().get_visible_rect()
		var top_left = camera.get_screen_center_position() - viewport_rect.size / 2.0 / camera.zoom.x
		var bottom_right = top_left + viewport_rect.size / camera.zoom.x
		mouse_coords = mouse_coords.clamp(top_left, bottom_right)
