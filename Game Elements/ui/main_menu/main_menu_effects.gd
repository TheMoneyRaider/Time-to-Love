...
func fragment_disruption():
	...
	if disruptive and get_viewport():
		var mouse_pos = Vector2(1920/2.0,1000/4.0)
		for frag in $BreakFX.get_children():
			frag.apply_force_frag(mouse_pos,stength)
		mouse_pos = Vector2(1920/3.0,1000/4.0)
		for frag in $BreakFX.get_children():
			frag.apply_force_frag(mouse_pos,stength)
		mouse_pos = Vector2(1920/4.0,1000/4.0)
		for frag in $BreakFX.get_children():
			frag.apply_force_frag(mouse_pos,stength)
		mouse_pos = Vector2(1920*2.0/3.0,1000/4.0)
		for frag in $BreakFX.get_children():
			frag.apply_force_frag(mouse_pos,stength)
		mouse_pos = Vector2(1920*3.0/4.0,1000/4.0)
		for frag in $BreakFX.get_children():
			frag.apply_force_frag(mouse_pos,stength)
...
func explode_ui():
	...
	for state in [Globals.MenuState.Western,Globals.MenuState.Space,Globals.MenuState.Medieval]:
		...
		for frag_poly in fragments_data:
			...
			frag.begin_break(frag_poly, the_ui, Vector2(0,0))
			frag.add_interactive_area(frag_poly, [] as Array[Button])
...
func load_fragments(path: String) -> void:
	...
	for fdata in container.fragments:
		...
		frag.begin_break(fdata.polygon, the_ui, Vector2(0,0))
		frag.add_interactive_area(fdata.polygon, [] as Array[Button])
...
func preload_all_textures():
	var states = generate_all_valid_ui_states([])
	for state in states:
...
func capture_all_ui_states():		
	print("capturing")
	var valid_states = generate_all_valid_ui_states([])
